...
public class AsyncCompilerAgent {
    ...
    AsyncCompilerResult compileAdHocPlan(AdHocPlannerWork work) {
        ...
        for (final String sqlStatement : work.sqlStatements) {
            ...
            // Overly long predicate expressions can cause a StackOverflowError
            // in one of two (or more?) code paths leading to different catch
            // blocks. The factors that determine which code path and catch
            // block will get activated appears to be platform sensitive for
            // reasons we do not entirely understand.  To generate a
            // deterministic error message regardless of these factors,
            // purposely uglify the message generated in this code path to
            // exactly match the output produced in the other code path.
            // The amount of preamble that code path generates is unfortunate,
            // but we purposely match it here.
                ...
                errorMsgs.add("Encountered stack overflow error. " +
                        "Try reducing the number of predicate expressions in the query. ACA");
            }
